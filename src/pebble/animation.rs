...
struct AnimContext {
    ...
    // It sits inside this box, so its address is stable for the animation's life.
...
impl Animation {
    ...
    pub fn new(update: impl FnMut(AnimationProgress) + 'static) -> Self {
        let mut ctx = Box::new(AnimContext {
            impl_: AnimationImplementation {
                setup:    None,
        ...
        let ctx_ptr: *mut AnimContext = &mut *ctx;
        ...
        interface::animation_set_handlers(raw, AnimationHandlers { started: None, stopped: None }, ctx_ptr as *mut u8);

        Animation { raw, _ctx: ctx }
    }
    ...
    pub fn on_stopped(&mut self, stopped: impl FnMut(bool) + 'static) {
        self._ctx.stopped = Some(Box::new(stopped));
        let ctx_ptr: *mut AnimContext = &mut *self._ctx;
